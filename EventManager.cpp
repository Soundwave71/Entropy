...
// Constructor: set focus true, and load bindings from key.cfg file.
EventManager::EventManager(): m_hasFocus(true) { LoadBindings(); }

//Destructors: iterate on every binding, free second member's memory and set it to nullptr.
...
//check for binding's name, if found, return error, else add the binding.
bool EventManager::AddBinding(Binding *l_binding) {
    if (m_bindings.find(l_binding->m_name) != m_bindings.end())
        return false;
    return m_bindings.emplace(l_binding->m_name, l_binding).second;
}
//search for binding's name, if found, free memory and erase the binding. else return error.
...
//checks all the bindings and set the event details accordingly if a match is found.
void EventManager::HandleEvent(sf::Event &l_event) {
    //Cycles through all the bindings.
    for(auto &b_itr : m_bindings) {
        //set current binding
        Binding* bind= b_itr.second;
        //searches for a match between all types of event and the binding code.
