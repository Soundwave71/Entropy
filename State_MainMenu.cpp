...
void State_MainMenu::OnCreate() {
    //loads textures, sets up rects, texts
    ...
    for (int i=0; i<3; ++i)
    {
        ...
        m_rects[i].setFillColor(sf::Color::Cyan);
...
void State_MainMenu::MouseClick(EventDetails *l_details) {
    ...
    for(int i=0; i < 3; ++i)
    {
        if(mousePos.x >= m_rects[i].getPosition().x - halfX && mousePos.x <= m_rects[i].getPosition().x - halfX &&
                mousePos.y >= m_rects[i].getPosition().y - halfY && mousePos.x <= m_rects[i].getPosition().y + halfY)
        {
            if(i==0)
            {
